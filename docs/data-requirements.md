...
- `GET /sensors/measurements/seqs/<type>` – Zeitreihen-Abruf für mehrere Sensoren gleichzeitig. Parameter: `filter[sensorIds]` (kommasepariert), `filter[tFilter][dateFrom]`/`filter[tFilter][dateTo]` (ISO-Zeitraum), `filter[resolution]` (bestätigt u. a. `"1 minute"`, `"15 minutes"`, `"2 days"`, `"1 month"`), `filter[tz]` (IANA-Zeitzone, z. B. `Europe/Berlin`). Antwortform: `data.attributes.datetimes: string[]` plus je Sensor-ID ein paralleles `data.attributes.<sensorId>: number[]` (Index-Korrespondenz zu `datetimes`, keine Objekt-Liste pro Zeitpunkt).
  ...
  - Bestätigt funktionsfähig: `energy_mm_counter_seqs` (counter-artige Sensoren) und `avg_mm_gauge_seqs` (gauge-artige Sensoren, z. B. `battery_soc`) – beide gegen echte Sensor-IDs getestet (01.09.2026).
...
**Konsequenz für CEMS:** Migrationen 032/033 mappen `energy_mm_counter_seqs` auf die kanonischen `_total`-Metriken (`energy_generation_total` etc., `aggregation_method=LAST`) – das ist mit diesem Befund fachlich falsch; korrekt wäre entweder (a) `interpolated_mm_counter_seqs` für die `_total`-Metriken zu verwenden, oder (b) `energy_mm_counter_seqs` stattdessen auf die bereits in `docs/canonical-metrics.md` vorgesehenen Intervallenergie-Metriken (`energy_generation`, `energy_consumption`, ... ohne `_total`, `aggregation_method=SUM`) umzumappen. **Noch nicht behoben** – betrifft die realen `VendorMetricMapping`-Zeilen des Piloten (nicht den CEMS-Code selbst) und die auf Zählerdifferenz basierende Logik in `src/application/curtailment/curtailment.service.ts` (`counterDiffKwh` liefert für `energy_mm_counter_seqs`-Daten aktuell ein falsches, meist stark unterschätztes Ergebnis, weil es nicht summiert, sondern differenziert). Vor produktivem Einsatz der `_total`-Metriken oder der Curtailment-Berechnung zwingend zu klären.
