...
**Nächtlicher Idle-/Startup-Sollwert bei `pv_setpoint_power` (real beobachtet, 02.09.2026):** Bei einem echten Kunden hält `active_power_setpoint` (nach Vorzeichen-/Unit-Konversion) über Nacht (durchgängig 18:15–04:15 UTC an zwei realen Tagen) einen konstanten, eingefrorenen Wert von `5.50010009765625 kW` – bit-identisch bei jedem 15-Minuten-Zeitpunkt und bei allen sieben Wechselrichtern gleichzeitig. Tagsüber folgt derselbe Sensor dagegen einer plausiblen, sich änderenden Kurve (5–25 kW, korreliert mit dem Sonnenstand). Interpretation: kein lebendiges Rundsteuerungssignal, sondern ein vendorseitig eingefrorener Idle-/Startup-Wert des Wechselrichters – nicht offiziell dokumentiert, nur aus dem Muster abgeleitet (ADR-004: nicht weiter interpretiert). **Konsequenz für CEMS:** `PV_SETPOINT_VS_ACTUAL_V1` verglich diesen Wert bisher gegen die reale Ist-Erzeugung (nachts korrekt 0 kW) und feuerte dadurch jede Nacht bei jedem Wechselrichter – ein systematischer False Positive, kein Betriebsproblem. Behoben (02.09.2026): die Regel prüft jetzt zusätzlich `expected_active_power` (wetterbasiert, PV_SYSTEM-Ebene) und überspringt den Vergleich, wenn Erzeugung physikalisch nicht plausibel ist (`isGenerationPhysicallyPlausible`, `src/application/auditor/rules.ts`) – kein Rateergebnis über die Bedeutung des Vendor-Werts selbst, nur eine Plausibilitätsschranke auf Basis bereits vorhandener Daten.
