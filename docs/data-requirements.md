...
- gauge-artig (mindestens `battery_soc` bestätigt mit `avg_mm_gauge_seqs`; die übrigen hier gelistet, aber noch nicht einzeln gegen den Gauge-Endpunkt getestet): `battery_soh`, `battery_soc`, `battery_dc_voltage`, `battery_dc_current`, `battery_dc_power`, `battery_max_temperature`, `battery_min_temperature`, `battery_reactive_power`, `battery_setpoint_power`, `pv_reactive_power`, `pv_setpoint_power`
