import { fetchForecast, type WeatherSlot } from "./client.js";
...
import type { ConnectorId, MeasurementPointId, TenantId } from "../../domain/shared/ids.js";
...
/**
 * Pulls a weather forecast for a site and writes both the raw weather variables (on the site's
 * weather MeasurementPoint) and, for every PV_SYSTEM asset with a valid configuration, the
 * derived expected AC power (pv-model.ts) as an expected_active_power Measurement — reusing the
 * existing MeasurementIngestionService unchanged (same principle as the Wendeware connector:
 * reuse the ingestion chain, don't duplicate it).
 *
...
export class OpenMeteoIngestService {
  ...
  async pull(
    ...
    if (site.latitude === null || site.longitude === null) {
      return this.empty("Site has no latitude/longitude configured yet");
    }

    const pvSystems = await this.deps.assets.findByTypeAndSite(tenantId, connector.siteId, "PV_SYSTEM");
    const configured: { asset: Asset; config: PvSystemConfiguration }[] = [];
    ...
    const reference = configured[0]?.config;
    ...
    for (const slot of slots) {
      for (const [field, metricKey] of Object.entries(WEATHER_METRIC_KEYS) as [keyof WeatherSlot, string][]) {
        await this.deps.measurementIngestion.ingest({
          ...
          quality: slot.timestamp <= now ? "MEASURED" : "ESTIMATED",
          connectorId,
    ...
    return {
      weatherPointsIngested,
      expectedPowerPointsIngested,
      pvSystemsConfigured: configured.length,
      pvSystemsSkipped: pvSystems.length - configured.length,
      skippedReason: null,
    };
  }
