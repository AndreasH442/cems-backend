/**
 * Runs one real pull against the Open-Meteo ERA5 Archive API for an already-onboarded site
 ...
 * Safe to re-run (upsert on natural key, same as the Wendeware connector). Archive has a 2-5 day
 * lag — very recent dates may return no data.
 *
 * Usage:
 *   npm run weather:pull-archive -- <tenantId> <connectorId> <weatherMeasurementPointId> <startDate> <endDate>
 *   (dates as YYYY-MM-DD)
 */
...
async function main(): Promise<void> {
  const [tenantIdArg, connectorIdArg, weatherPointIdArg, startDateArg, endDateArg] = process.argv.slice(2);
  if (!tenantIdArg || !connectorIdArg || !weatherPointIdArg || !startDateArg || !endDateArg) {
    console.error(
      "Usage: npm run weather:pull-archive -- <tenantId> <connectorId> <weatherMeasurementPointId> <startDate> <endDate>",
    );
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    console.log(
      `Pulling archive weather for tenant ${tenantId} / connector ${connectorId}, ${startDateArg}..${endDateArg} ...`,
    );
    const result = await ingest.pullArchive(tenantId, connectorId, weatherMeasurementPointId, startDateArg, endDateArg);
